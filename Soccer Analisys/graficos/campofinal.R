library(grid)
library(lattice)
library(fmsb)
library(plyr)
library(party)
library(reshape2)
library(gplots)
library(gtools)
require(plyr)


options(stringsAsFactors = FALSE)

tempFiles = list.files(path=paste(getwd(),"/inputcsv/",sep=""), pattern="*.csv")
ifile=1
#for (ifile in 1:length(tempFiles))
{
  dados = read.csv(paste(getwd(),"/inputcsv/",tempFiles[ifile], sep=""), sep = ";")
  
  titulo =strsplit(tempFiles[ifile], '[.]')[[1]][1]
  titulo = sub('"', '', titulo)
  
  #lista de todas os possiveis SDOs
  lista_sdo <-  c("F","PML","PMC","PMR", "CM", "PDL", "PDC", "PDR","CD", "PBL", "PBC", "PBR", "CB") 
  #transforma em matrix para contar zero em todos os sdos
  matrixAllSDO <- ldply (lista_sdo, data.frame)
  #ajusta nome das colunas
  colnames(matrixAllSDO) <- c("SDO2")
  matrixAllSDO = count(matrixAllSDO, c('SDO2'))
  matrixAllSDO[matrixAllSDO=="1"] <- 0
  
  
  #seleciona somente as colunas necessarias
  dados <- dados[c("SDO","ESC")]
  dados[dados==""] <- NA
  dados[is.na(dados)] <- ""
  
  
  dados["SDO2"] <- NA
  # soma as colunas SDO com ESC para obter a informacao de esquerda e direita
  dados["SDO2"] = paste(dados$SDO, substring(dados$ESC, 2), sep="" )
  
  dados["ESC"] =  substring(dados$ESC, 2)
  
  
  #agora retira a informacao de lado para os seguintes SDO: F, CM, CD e CB
  
  dados <- na.omit(dados)
  names(dados)
  temp = dados[dados$SDO2=="FR",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="FR",]$SDO2 = "F"
  }
  temp = dados[dados$SDO2=="FC",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="FC",]$SDO2 = "F"
  }
  temp = dados[dados$SDO2=="FL",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="FL",]$SDO2 = "F"
  }
  temp = dados[dados$SDO2=="CMC",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="CMC",]$SDO2 = "CM"
  }
  temp = dados[dados$SDO2=="CMR",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="CMR",]$SDO2 = "CM"
  }
  temp = dados[dados$SDO2=="CML",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="CML",]$SDO2 = "CM"
  }
  temp = dados[dados$SDO2=="CDC",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="CDC",]$SDO2 = "CD"
  }
  temp = dados[dados$SDO2=="CDR",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="CDR",]$SDO2 = "CD"
  }
  temp = dados[dados$SDO2=="CDL",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="CDL",]$SDO2 = "CD"
  }
  temp = dados[dados$SDO2=="CBC",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="CBC",]$SDO2 = "CB"
  }
  temp = dados[dados$SDO2=="CBR",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="CBR",]$SDO2 = "CB"
  }
  temp = dados[dados$SDO2=="CBL",]
  if (nrow(temp) > 0){
    dados[dados$SDO2=="CBL",]$SDO2 = "CB"
  }
  
  ### faz loop para ver se PMC, PDC e PBC contara com R ou L
  for(i in 1:nrow(dados)) {
    rowAtual <- dados[i,]
    
    
    if (as.character(rowAtual$SDO2) == "PDC" 
        ||as.character(rowAtual$SDO2) == "PMC" 
        ||as.character(rowAtual$SDO2) == "PBC" ){
      
      rowAnterior <- dados[i-1,]
      indiceVolta = 1
      while (rowAnterior$ESC == "C" ){
        rowAnterior <- dados[i-indiceVolta,]
        indiceVolta = indiceVolta + 1
      }
      dados[i,]$SDO2 = paste(substring(rowAtual$SDO2, 1,2), rowAnterior$ESC, sep="" )
      
    }
    
  }
  
  
  
  
  ### conta ocorrencias
  dados_freq_temp = count(dados, c('SDO2'))
  dados_freq_temp[dados_freq_temp==""] <- NA
  dados_freq <- na.omit(dados_freq_temp)
  
  
  dados_freq = merge(x = dados_freq, y = matrixAllSDO, by = c("SDO2") , all.y = TRUE)
  dados_freq[is.na(dados_freq)] <- 0
  
  
  dados_freq <- dados_freq[c("SDO2", "freq.x")]
  colnames(dados_freq) <- c("SDO","freq")
  
  pesos <- as.list(dados_freq['freq'])
  
  
  sdo <-dados_freq[["SDO"]]
  
  # gera matriz from to
  
  matrixFT <- data.frame(
    from=integer(),
    to=integer())
  
  for(i in 1:nrow(dados)) {
    myRow <- matrixFT[1,]
    rowAtual <- dados[i,]
    
    strFrom <- ""
    strTo <- ""
    
    if (i < nrow(dados)){
      if (!is.na(rowAtual$SDO2)){
        strFrom <- rowAtual$SDO2
        rowNext <- dados[1+i,]
        strTo <- rowNext$SDO2
        
        myRow$from = strFrom
        myRow$to = strTo
        
        matrixFT <- rbind(matrixFT, myRow)   
        
      }
      
    }
    
  }
  
  
  matrixFT_temp = count(matrixFT, c('from', 'to'))
  matrixFT_freq <- na.omit(matrixFT_temp)
  
  matrixFT_freq[matrixFT_freq==""] <- NA
  matrixFT_freq <- na.omit(matrixFT_freq)
  
  
  #retirando totais para o mesmo no
  #for(i in 1:NROW(pesos)) {
  # if (nrow(matrixFT_freq[matrixFT_freq$from == sdo[i] & matrixFT_freq$to == sdo[i],])> 0){
  #  pesos[i] = pesos[i] - matrixFT_freq[matrixFT_freq$from == sdo[i] & matrixFT_freq$to == sdo[i],]$freq
  #}
  
  #}
  
  #matrixFT_freq <- matrixFT_freq[order(-matrixFT_freq$freq), ]
  
  pesos <-dados_freq[["freq"]]
  dados_freq <- dados_freq[order(dados_freq$SDO), ]
  
  
  dados_freq['x'] = c(4.5,3.5,2,0, 4,4,4.5,3, 3,3,1,1,1)
  dados_freq['y'] = c(0,0,0,0,0,-1.5,1.5,0,-1.3,1.3,0,-1,1)
  dados_freq['myColor'] = c("yellow", "firebrick1", "firebrick4", "gold", 
                            "orange", "orange4", "plum2", "red",
                            "yellow3","chocolate1","cornsilk","darkorange", 
                            "gold4")
  
  xvalues = dados_freq[['x']]
  yvalues = dados_freq[['y']]
  myColor = dados_freq[['myColor']]
  
  
  dados_freq <- dados_freq[order(dados_freq$freq), ]
  
  dat <- data.frame(x=xvalues,
                    y=yvalues,
                    weight=pesos,
                    text=sdo
                    , stringsAsFactors=FALSE)
  
  cols <- colorRampPalette(myColor)(nrow(dat))
  
  library(png)
  m <- readPNG( "campodefutebol.png")
  rimg <- as.raster(m)
  
  my_lt_plot <- function(dataplot){
    
    xyplot(y~x,data=dat,groups=weight,
           xlim=extendrange(dat$x,f=.17),
           ylim=extendrange(dat$y,f=.1),
           scales=list(x=list(draw=FALSE), y=list(draw=FALSE)),
           main=titulo, 
           panel=function(x,y,groups,...){
             
             correcaoRaio = .010  
             
             grid.raster(rimg, x=0.5, y=0.5, just="center", width=1, gp=gpar(alpha=0.5))
             
             lapply(seq_along(x),function(i){
               if(groups[i] > 0){
                 grid.circle(x[i],y[i],
                             r=correcaoRaio*groups[i],
                             gp=gpar(fill=cols[i],lwd=2),
                             def='native')
                 
                 #grid.text(x[i],y[i],label=paste(dat$text[i],paste(x[i], y[i])),
                 grid.text(x[i],y[i],label=dat$text[i],
                           gp=gpar(cex=1,col='black'),
                           def='native')
                 
               }
             })
             
             
             
             
             
             #for(i in 1:10) {
             for(i in 1:nrow(matrixFT_freq)) {
               
               #  browser()
               rowFreqAtual <- matrixFT_freq[i,]
               indiceFrom = which(sdo == as.character(rowFreqAtual$from))
               indiceTo = which(sdo == as.character(rowFreqAtual$to))
               pesoSeta = rowFreqAtual$freq
               
               indiceMatrixFromX = dat[indiceFrom,]$x
               indiceMatrixToX = dat[indiceTo,]$x
               
               indiceMatrixFromY = dat[indiceFrom,]$y
               indiceMatrixToY = dat[indiceTo,]$y
               
               ajusteSetaFrom = correcaoRaio*groups[indiceFrom]
               ajusteSetaTo = correcaoRaio*groups[indiceTo]
               
               ajusteCurva = -.8;
               
               if(pesoSeta > 0){
                 if (indiceFrom != indiceTo){
                   
                   if (indiceMatrixFromX > indiceMatrixToX && indiceMatrixFromY < indiceMatrixToY){
                     #seta voltando cima
                     #browser()
                     xx <- c(x[indiceFrom]-ajusteSetaFrom/2, x[indiceFrom]-ajusteSetaFrom/2, 
                             x[indiceTo], x[indiceTo])
                     
                     yy <- c(y[indiceFrom]+ajusteSetaFrom/2, y[indiceFrom]+ajusteSetaFrom/2, 
                             y[indiceTo]-ajusteSetaTo/1.15, y[indiceTo]-ajusteSetaTo/1.15)
                     
                   }else if (indiceMatrixFromX > indiceMatrixToX && indiceMatrixFromY == indiceMatrixToY){
                     #seta voltando mesma altura
                     
                     xx <- c(x[indiceFrom]-ajusteSetaFrom, x[indiceFrom]-ajusteSetaFrom, 
                             x[indiceTo]+ajusteSetaTo, x[indiceTo]+ajusteSetaTo)
                     
                     yy <- c(y[indiceFrom], y[indiceFrom], 
                             y[indiceTo], y[indiceTo])
                     
                   }else if (indiceMatrixFromX > indiceMatrixToX && indiceMatrixFromY > indiceMatrixToY){
                     #seta voltando baixo
                     
                     
                     xx <- c(x[indiceFrom], x[indiceFrom], 
                             x[indiceTo]+ajusteSetaTo, x[indiceTo]+ajusteSetaTo)
                     
                     yy <- c(y[indiceFrom]-ajusteSetaFrom/2, y[indiceFrom]-ajusteSetaFrom/2, 
                             y[indiceTo], y[indiceTo])
                     
                     
                   }else if (indiceMatrixFromX == indiceMatrixToX && indiceMatrixFromY > indiceMatrixToY){
                     #seta mesmo lado baixo
                     
                     xx <- c(x[indiceFrom], x[indiceFrom]-ajusteSetaFrom, 
                             x[indiceTo]-ajusteSetaTo, x[indiceTo]-ajusteSetaTo)
                     
                     yy <- c(y[indiceFrom]+ajusteSetaFrom, y[indiceFrom]+ajusteCurva, 
                             y[indiceTo], y[indiceTo])
                     
                   }else if (indiceMatrixFromX < indiceMatrixToX && indiceMatrixFromY > indiceMatrixToY){
                     #seta indo baixo
                     ajusteCurva = ajusteCurva*-1
                     #browser()
                     xx <- c(x[indiceFrom], x[indiceFrom], 
                             x[indiceTo], x[indiceTo])
                     
                     yy <- c(y[indiceFrom]-ajusteSetaFrom/2, y[indiceFrom]-ajusteSetaFrom/2, 
                             y[indiceTo]+ajusteSetaTo, y[indiceTo]+ajusteSetaTo/1.1)
                     
                   }else if (indiceMatrixFromX < indiceMatrixToX && indiceMatrixFromY == indiceMatrixToY){
                     #seta indo mesma altura
                     
                     xx <- c(x[indiceFrom]+ajusteSetaFrom, x[indiceFrom], 
                             x[indiceTo]-ajusteSetaTo, x[indiceTo]-ajusteSetaTo)
                     
                     yy <- c(y[indiceFrom], y[indiceFrom]+ajusteCurva, 
                             y[indiceTo], y[indiceTo])
                     
                   }else if (indiceMatrixFromX < indiceMatrixToX && indiceMatrixFromY < indiceMatrixToY){
                     #seta indo cima
                     
                     xx <- c(x[indiceFrom]+ajusteSetaFrom/2, x[indiceFrom], 
                             x[indiceTo]-ajusteSetaTo, x[indiceTo]-ajusteSetaTo) #altura
                     
                     yy <- c(y[indiceFrom], y[indiceFrom], 
                             y[indiceTo], y[indiceTo]) # altura
                     
                   }else if (indiceMatrixFromX == indiceMatrixToX && indiceMatrixFromY < indiceMatrixToY){
                     #seta mesmo lado cima
                     
                     xx <- c(x[indiceFrom], x[indiceFrom]-ajusteSetaFrom, 
                             x[indiceTo]-ajusteSetaTo, x[indiceTo]-ajusteSetaTo)
                     yy <- c(y[indiceFrom]+ajusteSetaFrom, y[indiceFrom]+ajusteCurva, 
                             y[indiceTo], y[indiceTo])
                     
                   }
                   
                   corSeta = cols[indiceFrom]
                   grid.bezier(xx, yy,
                               gp=gpar(lwd=pesoSeta*.7, col=corSeta),
                               arrow=arrow( angle=20, type="open", length=unit(0.1, "inches")),
                               def='native')
                   
                   
                 }else{
                   #seta para o proprio no
                   
                   xx <- c(x[indiceFrom], x[indiceFrom], 
                           x[indiceTo]-ajusteSetaTo, x[indiceTo]-ajusteSetaTo)
                   
                   yy <- c(y[indiceFrom]+ajusteSetaFrom/1.2, y[indiceFrom]+ajusteSetaFrom+.2, 
                           y[indiceTo]+1, y[indiceTo])
                   
                   corSeta = cols[indiceFrom]
                   grid.bezier(xx, yy,
                               gp=gpar(lwd=pesoSeta*.3, col="black"),
                               arrow=arrow( angle=50, type="closed", length=unit(0.1, "inches")),
                               def='native')
                   
                 }
                 
                 
                 
                 
               }
               
             }         
             
           })
    
  }
  
  
  #png(filename=paste(getwd(),"/outputfield/",titulo,".png", sep=""), 
  png(filename=paste(getwd(), "/outputfield/Lattice_PNG_pt12.png", sep=""), 
      units="in", 
      width=5, 
      height=4, 
      res=96)
  my_lt_plot(dataplot)
  dev.off()
  
} 